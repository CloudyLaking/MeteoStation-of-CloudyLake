...
## V2.4.5 · 2026-09-05 — 天气图阅读与标注布局

- 天气图占满主内容宽度，探空工具移至图下方；新增适应屏幕、查看细节与原图入口。
- 等值线数字按标注实际边界避让，不再隐藏热带系统周围大范围数字；在最终地图比例下计算文字位置。
- 手机默认完整显示地图，细节模式可横向查看；总览将站点数值收为可点击站点，减少主图干扰。
- 更新自动诊断与单位说明，保留图外图例及英文图内文字。
