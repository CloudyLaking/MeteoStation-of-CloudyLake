...
def search_storms(
    ...
    matches = [
        ...
        for storm in storms
        if not query
        or query in str(storm.get("sid", "")).casefold()
        or query in str(storm.get("name", "")).casefold()
    ]
    matches.sort(
        key=lambda storm: (
            int(storm.get("season", 0)),
            str(storm.get("sid", "")),
        ),
        reverse=True,
    )
    ...
    return [
        {key: storm.get(key) for key in fields}
        for storm in matches[:limit]
    ]
...
def _sample(
    ...
    if len(points) <= count:
        return points
    return [
        points[round(index * (len(points) - 1) / (count - 1))]
...
def _track_distance(
    ...
    for index in range(count):
        left_point = left[
            round(index * (len(left) - 1) / (count - 1))
        ]
        right_point = right[
            round(index * (len(right) - 1) / (count - 1))
        ]
        distances.append(
            haversine_km(
...
def analogs(
    ...
    storms = load_index(str(path.resolve()))
    target = next(
        (storm for storm in storms if storm.get("sid") == sid),
        None,
    )
    if target is None:
        return None
        ...
        ranked.append(
            {
                ...
                "components": {
                    ...
                    "season": round(season_score, 4),
                    "era5_environment": None,
                },
