...
| 验收项 | 初检状态 |
| --- | --- |
| 1. IFS/AIFS 查询 200 且无静默过期 | ❌ 更新期出现 500；状态文件与文件系统不一致 |
| 2. 更新缓存期间连续请求无 500 | ❌ TOCTOU 竞争 |
| 3. `/health/ready` 数据过期时正确失败 | ❌ 仅有 `/api/v1/health`（只报存活） |
| 4. 首页首屏资源 < 3 MB | ✅ 当前 253 KB，但无压缩/缓存/字体分包 |
| 5. 全部基准尺寸无重叠 | 未测（需截图回归） |
| 6. 24h 实况缓存 1 秒内返回 | ✅ 31 ms（热缓存） |
| 7. WIS 未来报文隔离 | ❌ 未实现；原始目录跨 6 个月 |
| 8. 公开天气图非未标识预览 | ❌ 首页静默展示 development-preview |
| 9. 天气图置信等级有案例验证 | ❌ 未实现 |
| 10. 统一模板与设计系统 | ❌ 七个页面重复顶栏/导航/页脚 |
| 11. 一键回滚 | ❌ 无 releases/<commit> 结构 |
| 12. 文档与线上一致 | ❌ 部分过时 |
