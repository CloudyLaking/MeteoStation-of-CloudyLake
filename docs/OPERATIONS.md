...
run_web.py
  ├── 读取本地原始资料缓存
  ├── 提供交互探空所需数据
  ├── 调用原版 q-weather 逐小时链路并以内存方式生成静态图
  └── 按查询读取 q-weather 实时状态
```
...
`/observations` 不建立地面实况产品归档。用户可提交 WMO 站号、中文站名或“省份+站名”，服务端通过 `Basic_function/station info.txt` 解析为唯一站点后：

1. “过去 24h”调用原脚本的 q-weather `today` 页面；
2. “历史日期”调用原脚本的 q-weather `history` 页面；
3. 保持原脚本的数据表处理、历史倒序和逐小时字段；
4. 调用未改图形形式的 `Basic_function/History_ObsStation.py`，在内存中生成 PNG；
5. 将 PNG 直接返回浏览器，响应标记 `Cache-Control: no-store`。

页面另行调用 q-weather realtime API 显示当前状态。两条链路均不在服务器保留地面图或查询结果文件。
