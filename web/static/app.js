  ...
  for (const button of mapSoundingStations.children) {
    ...
    button.innerHTML = `
      <svg viewBox="0 0 74 74" aria-hidden="true">
        ${windBarb}
        <circle cx="37" cy="37" r="3.2" fill="${statusColor}"></circle>
        <text class="station-model__temperature" x="29" y="32" text-anchor="end" font-size="6.5" font-weight="650">${formatValue(level?.temperature_c)}</text>
        <text class="station-model__dewpoint" x="29" y="47" text-anchor="end" font-size="6.5" font-weight="650">${formatValue(level?.dewpoint_c)}</text>
        <text class="station-model__secondary" x="45" y="32" font-size="5.8" font-weight="600">${secondaryValue}</text>
        <text class="station-model__id" x="44" y="47" font-size="5.2">${escapeHtml(station?.wmo_id ?? button.dataset.stationId)}</text>
      </svg>
    `;
