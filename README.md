...
`/observations` 沿用 q-weather `today/history` 逐小时数据源，但查询接口只解析并返回统一字段，不再为每次访问启动高分辨率 Matplotlib。浏览器使用共用 SVG 渲染器即时绘制，并可在本地导出 PNG；实测 58362 当天返回 24 条记录。网页进程复用 q-weather 长连接，按站点为逐小时表设置独立并发锁，并将当天统一序列保留在内存一小时；58362 在启动后后台预热。实时状态仍单独读取，不会被逐小时缓存替代。页面允许输入 WMO 站号、中文站名或“省份+站名”，提供“过去 24h”和“历史日期”两种查询，并加入华东、华北、东北、华中、华南、西南、西北七个可点击站点分区图。未查询时优先显示地区地图，查询后实时状态与图像会移到输入框下方，避免空白占位框和无效间距。
